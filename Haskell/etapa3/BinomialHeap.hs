...
insertTree t [] = [t]
insertTree EmptyTree ts = ts
insertTree t@(Node p1 k1 c1) (t2 : ts) = case t2 of
  EmptyTree -> Node p1 k1 c1 : ts
  Node p2 k2 c2 ->
    if length c1 == length c2
      then EmptyTree : insertTree (attach t t2) ts
      else t : t2 : ts
...
insert p k (BinomialHeap size tl) = BinomialHeap (size + 1) (insertTree new tl)
  where
    new = Node p k []
...
findMin (BinomialHeap _ tl) = let mt = minimumBy (compare `on` prio) [t | t <- tl, isTree t] in Just (prio mt, key mt)
  where
...
mergeTrees t [] = t
mergeTrees t1 t2 =
  let (remCarry, list) = mapAccumL funcAccum EmptyTree $ zipExtend EmptyTree EmptyTree t1 t2
   in list ++ [remCarry]
  where
    funcAccum carry (t1, t2) = (updateCarry, attached)
      where
        (updateCarry, attached)
          | isEmpty t1 && isEmpty t2 = (EmptyTree, carry)
          | isEmpty t1 = if isEmpty carry then (EmptyTree, t2) else (attach t2 carry, EmptyTree)
          | isEmpty t2 = if isEmpty carry then (EmptyTree, t1) else (attach t1 carry, EmptyTree)
          | otherwise = (attach t1 t2, carry)
...
merge (BinomialHeap size1 tl1) (BinomialHeap size2 tl2) = BinomialHeap (size1 + size2) (mergeTrees tl1 tl2)
...
isolate _ [] = []
isolate x (h : t) = (h, if null t then t else x : t) : map (Data.Bifunctor.second (h :)) (isolate x t)
...
removeMin heap = case trees heap of
  [] -> emptyHeap
  Node prio key children : ts ->
    merge newHeap emptyHeap
    where
        childTrees = isolate (Node prio key children) ts
        newHeap = BinomialHeap (size heap - 1) . mergeTrees (reverse children) . map fst $ childTrees
...
instance (Show p, Show k) => Show (BinomialTree p k) where
  show t = showTree t "\n"
    where
      showTree EmptyTree indent = "*"
      showTree (Node p k children) indent =
        show p ++ " (" ++ show k ++ ")" ++
        concatMap (showTree' indent) children
      showTree' indent child =
        indent ++ "  " ++ showTree child (indent ++ "  ")

instance (Show p, Show k) => Show (BinomialHeap p k) where
    show heap = case heap of
      BinomialHeap _ [] -> ""
      BinomialHeap _ trees -> init $ unlines $ map show trees
...
instance Functor (BinomialTree p) where
    ...
    fmap f = aux
      where
        aux EmptyTree = EmptyTree
        aux (Node p k children) = Node p (f k) (fmap aux children)
...
instance Functor (BinomialHeap p) where
    ...
    fmap f heap = BinomialHeap (size heap) $ map (fmap f) (trees heap)

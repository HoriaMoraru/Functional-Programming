...
(define (match person engagements pref1 pref2 queue)
  (let loop-match ((pers person)
                   (eng engagements)
                   (lov (get-pref-list pref1 person)))
    (if 
      (null? lov) (append eng (list (cons #f pers)))
       (let* ((lover (car lov))
              (pref-opp (get-pref-list pref2 lover))
              (partner (get-partner eng lover)))
         (cond
            ((member? lover queue) (loop-match pers eng (cdr lov)))
            ((equal? partner #f) (update-engagements eng lover pers)) 
            ((preferable? pref-opp pers partner) (loop-match partner (update-engagements eng lover pers) (get-pref-list pref1 partner)))
            (else (loop-match pers eng (cdr lov))))))))
...
(define (path-to-stability engagements mpref wpref queue)
  (let pts-loop ((eng engagements) (q queue))
    (if (null? q)
        eng
        (let* ((current (car q))
               (sex (if (member? current (get-men mpref)) #t #f)) ; sexul va fi #t pentru barbat si #f pentru femeie . f de la femeie nu de la fals , femeile nu sunt false !
               (reverse-eng (map (λ (pair) (cons (cdr pair) (car pair))) eng)))
          (if (equal? sex #t)
              (pts-loop (match current eng mpref wpref (cdr q)) (cdr q))
              (pts-loop (map (λ (pair) (cons (cdr pair) (car pair))) (match current reverse-eng wpref mpref (cdr q))) (cdr q)))))))
...
(define (update-stable-match engagements mpref wpref)
  (let* ((uc (get-unstable-couples engagements mpref wpref))
         (q (get-couple-members uc))
         (room (remove* uc engagements)))
    (path-to-stability room mpref wpref q)))
...
(define (build-stable-matches-stream pref-stream)
  (if (stream-empty? pref-stream)
      empty-stream
      (let* ((pref-in1 (car (stream-first pref-stream)))
             (pref-in2 (cdr (stream-first pref-stream)))
             (pref1 (car (stream-first (stream-rest pref-stream))))
             (pref2 (cdr (stream-first (stream-rest pref-stream))))
             (init (gale-shapley pref-in1 pref-in2)))
        (stream-append (stream-cons init (list (update-stable-match init pref1 pref2))) (build-stable-matches-stream (stream-rest (stream-rest pref-stream)))))))

...
(define (get-women-tail womlist wpref)
  (if (null? wpref)
      (reverse womlist)
      (get-women-tail
      (append (list(car (car wpref))) womlist)
      (cdr wpref))))
...
(define (get-partner engagements person)
  (cond ((null? engagements) #f)
        ((member-pair? (car engagements) person) (cdr (car engagements)))
        (else (get-partner (cdr engagements) person))
      ))


(define (member-pair? pair person)
  (cond
    ((null? person) #f)

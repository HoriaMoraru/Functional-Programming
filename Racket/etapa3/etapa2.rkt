...
(define (get-pref-list pref person)
  (foldr (λ (plist acc)
           (if (equal? (car plist) person)
           (append (cdr plist) acc)
           acc))
         '()
         pref))
...
(define (preferable? pref-list x y)
   (member? y (drop pref-list (index-of pref-list x))))

  (define (member? elem pref-list)
  (list? (member elem pref-list)))
...
(define (get-partner engagements person)
  (define (get-partner-aux) (find-first (λ (pair)
                (cond
                 ((null? person) #f)
                 ((eq? (car pair) person) #t)
                 (else #f)))
              engagements))
...
(define (change-first p L val)
  (change-first-tail p L val '()))

(define (change-first-tail p L val acc)
  (cond
    ((null? L) (reverse acc))
    ((p (car L)) (append (reverse acc) (list val) (cdr L)))
    (else (change-first-tail p (cdr L) val (cons (car L) acc)))))
...
(define (stable-match? engagements mpref wpref)
 (define (stable-map)
   (map (λ (pair)
         (better-match-exists? (cdr pair)
                               (car pair)
                               (get-pref-list mpref (cdr pair))
                               wpref engagements))
       engagements))
  (if (index-of (stable-map) #t)
      #f
      #t))
